...
Every run executes the full poll (Phase 0 to 5) from scratch. Earlier runs recorded in `/app/task-context/memory.md` or in the ledger are context for interpretation only, never a reason to skip a phase, reuse an old analysis or stop early; the ledger decides what is new.
